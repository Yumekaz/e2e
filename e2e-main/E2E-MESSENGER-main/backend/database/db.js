...
async function initializeDatabase() {
  ...
  db.run(`
    ...
    -- Deleted messages tracking (for "delete for me" feature)
    ...
    CREATE INDEX IF NOT EXISTS idx_deleted_messages_user ON deleted_messages(username);
  `);
...
function deleteMessage(messageId, deleteType, deletedBy) {
  if (deleteType === 'forEveryone') {
    // Soft delete - mark as deleted for everyone
    return runQuery(`
      UPDATE messages
      SET state = 'deleted', encrypted_data = '[deleted]', iv = '[deleted]'
      WHERE message_id = ?
    `, [messageId]);
  } else {
    // Delete for me - we need to track which users have deleted the message
    // For simplicity, we'll just mark it in a separate table or use a JSON field
    // Here we'll use a simple approach - add to a deleted_messages table
    return runQuery(`
      INSERT OR IGNORE INTO deleted_messages (message_id, username, deleted_at)
      VALUES (?, ?, datetime('now'))
    `, [messageId, deletedBy]);
  }
}
