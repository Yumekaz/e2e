...
function createMessageHandler(io, socket, state) {
   ...
   * Delete message
   */
  socket.on('delete-message', ({ roomId, messageId, deleteType, username }) => {
    const user = users.get(socket.id);
    if (!user) {
      socket.emit('error', { message: 'Not registered' });
      return;
    ...
    if (deleteType === 'forEveryone' && message.sender_username !== user.username) {
      socket.emit('error', { message: 'Can only delete your own messages' });
      return;
    }

    // Delete from database
    db.deleteMessage(messageId, deleteType, user.username);
    ...
    logger.debug('Message deleted', { messageId, deleteType, deletedBy: user.username });
  });

  /**
   * Screenshot detected
   */
  socket.on('screenshot-detected', ({ roomId, username }) => {
    const user = users.get(socket.id);
    if (!user) return;
    ...
    if (db.isRoomMember(roomId, user.username)) {
      ...
      socket.to(roomId).emit('screenshot-warning', { username: user.username });
    }
  });
}
